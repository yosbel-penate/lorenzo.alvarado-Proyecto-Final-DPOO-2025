...
public class Level1 {
    ...
    private void spawnEnemies() {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            DroneCombate droneLogic = new DroneCombate();
            EnemyEntity drone = new EnemyEntity(droneLogic, playerEntities, turnManager);

            Entity fxglEntity = FXGL.spawn("enemy",
                    new SpawnData((int) spawnPos.getX(), (int) spawnPos.getY())
                            .put("type", "dronecombate")
                            .put("logic", droneLogic)
            );

            drone.setEntity(fxglEntity);
            drone.getLogic().setPosition((int) fxglEntity.getX() / 56, (int) fxglEntity.getY() / 56);

            enemies.add(drone);
            turnManager.addEnemy(drone);
        ...
        for (int i = 0; i < 5; i++) {
            ...
            AranaCosmica aranaLogic = new AranaCosmica(getHeroesFromPlayers());
            EnemyEntity arana = new EnemyEntity(aranaLogic, playerEntities, turnManager);

            Entity fxglEntity = FXGL.spawn("enemy",
                    new SpawnData((int) spawnPos.getX(), (int) spawnPos.getY())
                            .put("type", "arana")
                            .put("logic", aranaLogic)
            );

            arana.setEntity(fxglEntity);
            arana.getLogic().setPosition((int) fxglEntity.getX() / 56, (int) fxglEntity.getY() / 56);

            enemies.add(arana);
            turnManager.addEnemy(arana);
            usedPositions.add(spawnPos);
