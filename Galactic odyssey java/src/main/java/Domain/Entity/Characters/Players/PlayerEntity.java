...
public class PlayerEntity {
    ...
    private boolean bombaEnEspera = false;
    ...
    public void spawnPlayer() {
        player = FXGL.entityBuilder()
                ...
                .view(FXGL.texture(hero.getTextureName(), 56, 56))
...
// Verificar si hay un enemigo en la posición futura
    ...
    private void initInput() {
        ...
        getInput().addAction(new UserAction("LanzarBombaACasilla") {
            @Override
            protected void onActionBegin() {
                if (!bombaEnEspera) return;  // Solo si está esperando el clic
                ...
                // Convertir posición del jugador a tiles
                Point2D playerPos = new Point2D(player.getX() / TILE_SIZE, player.getY() / TILE_SIZE);
                Point2D targetPos = new Point2D(tileX, tileY);

                // Calcular distancia Manhattan (opcional, puedes usar distance euclidiana si prefieres)
                double dist = Math.abs(playerPos.getX() - targetPos.getX()) + Math.abs(playerPos.getY() - targetPos.getY());

                if (dist <= 5) {
                    Optional<Item> maybeBomba = GlobalInventory.getItems().stream()
                            .filter(item -> item instanceof BombaPlasma)
                            .findFirst();

                    if (maybeBomba.isPresent()) {
                        BombaPlasma bomba = (BombaPlasma) maybeBomba.get();
                        bomba.use(targetPos, turnManager.getEnemies());
                        GlobalInventory.removeItem(bomba);
                        if (hud != null) hud.updateInventoryImages();

                        System.out.println("Bomba lanzada a (" + tileX + ", " + tileY + ")");
                        turnManager.endTurn();
                        turnManager.endTurn();
                    } else {
                        System.out.println("No tienes ninguna bomba en el inventario.");
                    }
                } else {
                    System.out.println("¡Muy lejos para lanzar la bomba!");
                }

                bombaEnEspera = false; // Terminamos la espera
            }
        }, MouseButton.SECONDARY);

// Acción para activar el modo de selección con la tecla 3
        getInput().addAction(new UserAction("Use Bomb") {
            @Override
            protected void onActionBegin() {
                if (bombaEnEspera) {
                    System.out.println("Ya estás en modo selección de bomba. Haz click para lanzar.");
                    return;
                }
                System.out.println("Selecciona una casilla para lanzar la bomba...");
                bombaEnEspera = true;
            }
        }, KeyCode.DIGIT3);
