...
public class MenuController {
    ...
    private void showLevelSelectorScreen(List<String> selectedCharacters) {
        LevelSelectorController levelSelector = new LevelSelectorController(
                ...
                selectedCharacters,
                levelName -> {
                    // ✅ Ejecuta el nivel correcto según el nombre
                    switch (levelName) {
                        case "ESTACIÓN ESPACIAL":
                            new Level2().initialize(selectedCharacters);
                            break;
                        case "CIUDAD FLOTANTE":
                            Level3 level3 = new Level3(this::showMenu);
                            level3.initialize(selectedCharacters);
                            break;

                        case "LA LUNA OLVIDADA":
                        default:
                            new Level1().initialize(selectedCharacters);
                            break;
                    }
                    hideMenu(); // Oculta el menú una vez comienza el juego
                }
        );
